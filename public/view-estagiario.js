...
$(document).ready(async function () {
    ...
    async function renderQuestions() {
        ...
        let responsesObj = existingResponses;
        ...
        for (const [key, value] of Object.entries(responsesObj)) {
            let questionCard = `
                <div class="question-card mb-3 p-3 border rounded">
                    <div class="fw-bold mb-1 text-primary">Questão ${escapeHtml(key)}</div>
                    <div class="mb-2">${escapeHtml(value.pergunta)}</div>
                    <div class="p-2 bg-light rounded shadow-sm">
                        <strong>Resposta:</strong> ${escapeHtml(value.valor)}
    ...
    window.generateDeclaracao = async function () {
        try {
            // Fetch estagiario
            const estRes = await authenticatedFetch(`/estagiarios/${id}`);
            if (!estRes.ok) throw new Error('Falha ao carregar estagiário');
            const est = await estRes.json();

            // Fetch full aluno data (cpf, identidade, orgao)
            const alunoRes = await authenticatedFetch(`/alunos/${est.aluno_id}`);
            if (!alunoRes.ok) throw new Error('Falha ao carregar aluno');
            const aluno = await alunoRes.json();

            // Verify if the est.supervisor_id has a valid value
            if (!est.supervisor_id) {
                throw new Error('Supervisor não definido para este estagiário.');
                return;
            }
            const supRes = await authenticatedFetch(`/supervisores/${est.supervisor_id}`);
            if (!supRes.ok) throw new Error('Falha ao carregar supervisor');
            const supervisor = await supRes.json();

            // Get all the variables from the alunos and estagiario
            const nome = aluno.nome;
            const cpf = aluno.cpf || ' s/d';
            const identidade = aluno.identidade || ' s/d';
            const orgao = aluno.orgao || ' s/d';
            const registro = aluno.registro;
            const nivel = est.nivel;
            const supervisor_nome = supervisor.nome;
            const supervisor_cress = supervisor.cress || ' s/d';
            const supervisor_regiao = supervisor.regiao || ' s/d';
            const instituicao_nome = est.instituicao_nome;
            ...
            doc.setFontSize(14);
            doc.text('Coordenação de Estágio', 105, y, { align: 'center' });
            y += 10;
            doc.text('Declaração de Estágio Curricular', 105, y, { align: 'center' });
            y += 14;
            ...
            const body =
                'Declaramos que o(a) aluno(a) ' + nome +
                ' inscrito(a) no CPF sob o nº ' + cpf +
                ' e no RG nº ' + identidade +
                ' expedido por ' + orgao +
                ', matriculado(a) no Curso de Serviço Social da' +
                ' Universidade Federal do Rio de Janeiro com o número ' + registro +
                ', estagiou na instituição ' + instituicao_nome +
                ', com a supervisão profissional do(a) Assistente Social ' + supervisor_nome +
                ' registrado(a) no CRESS com o número ' + supervisor_cress + ' da ' + supervisor_regiao +
                'ª região, no semestre ' + periodo +
                ', com uma carga horária de ' + ch + ' horas.';

            const bodyLines = doc.splitTextToSize(body, maxW);
            doc.text(ml, y, bodyLines, { maxWidth: maxW, align: 'justify' });
            y += doc.getTextDimensions(bodyLines).h + 8;

            const body2 =
                'As atividades desenvolvidas correspondem ao ' +
                'nível ' + nivel + ' do currículo da Escola de Serviço Social da UFRJ.';
            const body2Lines = doc.splitTextToSize(body2, maxW);
            doc.text(ml, y, body2Lines, { maxWidth: maxW, align: 'justify' });
            ...
            doc.text(supervisor_nome, col3, y);
            y += 6;

            doc.text('Escola de Serviço Social', col1, y);
            doc.text('DRE: ' + registro, col2, y, { align: 'center' });
            doc.text('CRESS: ' + supervisor_cress + ' da ' + supervisor_regiao + ' Região', col3, y);
            y += 6;

            doc.text('UFRJ', col1, y);

            doc.save(`declaracao_${nome}_${periodo}.pdf`);
        } catch (error) {
    ...
    // Print atividades
    window.printAtividadesReport = async function () {
        try {
            ...
            const atividadesRes = await authenticatedFetch(`/estagiarios/${id}/atividades`);
            if (!atividadesRes.ok) throw new Error('Falha ao carregar atividades');
            const atividadesRaw = await atividadesRes.json();
            const atividades = Array.isArray(atividadesRaw) ? atividadesRaw : [];

            let supervisor = null;
            if (est && est.supervisor_id) {
                try {
                    const supRes = await authenticatedFetch(`/supervisores/${est.supervisor_id}`);
                    if (supRes.ok) supervisor = await supRes.json();
                } catch (_) {
                }
            }

            const alunoNome = String((est && (est.aluno_nome || est.alunoNome)) || '');
            const nivel = String((est && est.nivel) || '');
            const periodo = String((est && est.periodo) || '');
            const instituicaoNome = String((est && (est.instituicao_nome || est.instituicaoNome)) || '');
            const professorNome = String((est && (est.professor_nome || est.professorNome)) || '');

            const supervisorNome = String((supervisor && supervisor.nome) || (est && est.supervisor_nome) || '');
            const supervisorCress = String((supervisor && supervisor.cress) || (est && est.supervisor_cress) || 's/d');
            const supervisorRegiao = String((supervisor && supervisor.regiao) || (est && est.supervisor_regiao) || 's/d');
            ...
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(14);
            doc.text('Coordenação de estágio', pageWidth / 2, y, { align: 'center' });
            y += 8;

            doc.setFontSize(12);
            doc.text(`Relatório de atividades do aluno(a): ${alunoNome}`, marginX, y);
            y += 10;
            ...
            const writeKV = (label, value) => {
                ...
                const lines = doc.splitTextToSize(text, boxW - 8);
                doc.text(lines, boxX + 4, cursorY);
                cursorY += lines.length * lineGap;
            };

            writeKV('Nível:', nivel);
            writeKV('Período:', periodo);
            writeKV('Instituição:', instituicaoNome);
            writeKV('Supervisor(a):', supervisorNome);
            writeKV('CRESS:', supervisorCress);
            writeKV('Região:', supervisorRegiao);
            writeKV('Professor(a):', professorNome);

            const boxH = (cursorY - boxY) + 3;
            ...
            const colFimW = 18;
            const colHorW = 22;
            const colAtvW = boxW - (colDiaW + colIniW + colFimW + colHorW);

            const xDia = boxX;
            const xIni = xDia + colDiaW;
            const xFim = xIni + colIniW;
            const xHor = xFim + colFimW;
            const xAtv = xHor + colHorW;

            const headerH = 8;

            const ensureSpace = (needH) => {
                if (y + needH <= pageHeight - marginBottom) return;
                doc.addPage();
                y = marginTop;
                drawTableHeader();
            };
            ...
            const drawTableHeader = () => {
                ...
                doc.text('Dia', xDia + 2, y + 5.5);
                doc.text('Início', xIni + 2, y + 5.5);
                doc.text('Final', xFim + 2, y + 5.5);
                doc.text('Horário', xHor + 2, y + 5.5);
                doc.text('Atividade', xAtv + 2, y + 5.5);
                y += headerH;
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(9);
            };
            ...
            for (const a of atividades) {
                const dia = a && a.dia ? new Date(a.dia).toLocaleDateString('pt-BR') : '';
                const inicio = String((a && a.inicio) || '');
                const fim = String((a && a.final) || '');
                const horario = String((a && a.horario) || '');
                const atividadeTxt = String((a && a.atividade) || '');

                totalMinutes += parseToMinutes(horario);

                const atividadeLines = doc.splitTextToSize(atividadeTxt, colAtvW - 4);
                const lineCount = Math.max(1, atividadeLines.length);
                const rowH = Math.max(7, lineCount * 5);

                ensureSpace(rowH + 2);

                doc.setDrawColor(220);
                doc.rect(boxX, y, boxW, rowH);
                doc.line(xIni, y, xIni, y + rowH);
                doc.line(xFim, y, xFim, y + rowH);
                doc.line(xHor, y, xHor, y + rowH);
                doc.line(xAtv, y, xAtv, y + rowH);

                doc.text(dia, xDia + 2, y + 5);
                doc.text(inicio, xIni + 2, y + 5);
                doc.text(fim, xFim + 2, y + 5);
                doc.text(horario, xHor + 2, y + 5);
                doc.text(atividadeLines, xAtv + 2, y + 5);

                y += rowH;
            }

            const totalStr = formatMinutes(totalMinutes);
            ensureSpace(12);

            doc.setFont('helvetica', 'bold');
            doc.rect(boxX, y, boxW, 10);
            doc.text('Total', xDia + 2, y + 6.5);
            doc.text(totalStr, xHor + 2, y + 6.5);
            doc.setFont('helvetica', 'normal');
            y += 16;
            ...
            console.error('Erro ao imprimir relatório de atividades:', error);
            alert(`Erro ao imprimir relatório de atividades: ${error.message}`);
        }
    };
    ...
    window.generateCertificate = async function () {
        try {
            ...
            const alunoNome = String((est && est.aluno_nome) || '');
            const supervisorNome = String((est && est.supervisor_nome) || ' s/d');
            ...
            doc.text('Estudante', 105, height + 30, { align: 'center' });
            doc.text('Supervisor(a)', 150, height + 30, { align: 'left' });

            doc.text(alunoNome, 105, height + 35, { align: 'center' });
            doc.text(supervisorNome, 150, height + 35, { align: 'left' });

            // Save
            doc.save(`certificado_${alunoNome}_${est.periodo}.pdf`);
