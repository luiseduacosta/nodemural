...
window.imprime = async () => {
    try {
        ...
        // Logo: PNG em base64 é adequado para brasão (bordas nítidas, transparência). JPEG é melhor para fotos;
        // SVG no jsPDF costuma exigir extensão (svg2pdf). Alternativa: arquivo estático em /public + fetch + addImage.
        ...
        const appendParagraph = (text, fontSize, yPos) => {
            ...
            const textOpts = { maxWidth: maxTextW, align: 'justify' };
            doc.text(text, marginX, yPos, textOpts);
            const blockH = doc.getTextDimensions(text, { maxWidth: maxTextW }).h;
            return yPos + blockH + 6;
        };
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'normal');
        doc.text(`Rio de Janeiro, ${dia} de ${mes} de ${ano}. }`, pageWidth - marginX, y + 8, { align: 'right' });
        ...
        doc.text('UFRJ', pageWidth / 2, y + 46, { align: 'center' });
