// Test script for instituicao CRUD operations
// Note: The instituicao table requires several fields to be NOT NULL:
// endereco, bairro, municipio, cep, telefone
// These must be provided even as empty strings

import axios from 'axios';

const BASE_URL = 'http://localhost:3333/instituicao';

async function verify() {
    try {
        console.log('1. Creating new instituicao...');
        const createPayload = {
        ...
        console.log('\n4. Deleting instituicao...');
        await axios.delete(`${BASE_URL}/${newId}`);
        try {
            await axios.get(`${BASE_URL}/${newId}`);
        } catch (err) {
            if (err.response && err.response.status === 404) {
                console.log('Instituicao correctly not found after delete.');
            } else {
                throw err;
            }
        }
