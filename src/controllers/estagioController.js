...
export const createEstagio = async (req, res) => {
    try {
        ...
        const estagio = await Estagio.create(instituicao, cnpj, beneficio, areainstituicoes_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, avaliacao, observacoes);
...
export const updateEstagio = async (req, res) => {
    try {
        const { id } = req.params;
        const { instituicao, cnpj, beneficio, areainstituicoes_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, avaliacao, observacoes } = req.body;
        const success = await Estagio.update(id, instituicao, cnpj, beneficio, areainstituicoes_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, avaliacao, observacoes);
        if (!success) {
            return res.status(404).json({ error: 'Estagio not found' });
        }
        res.status(204).end();
