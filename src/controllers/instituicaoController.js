...
export const createInstituicao = async (req, res) => {
    try {
        const { instituicao, cnpj, natureza, email, beneficios, area_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, observacoes } = req.body;
        const newInstituicao = await Instituicao.create(instituicao, cnpj, natureza, email, beneficios, area_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, observacoes);
...
export const updateInstituicao = async (req, res) => {
    try {
        const { id } = req.params;
        const { instituicao, cnpj, natureza, email, beneficios, area_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, observacoes } = req.body;
        const success = await Instituicao.update(id, instituicao, cnpj, natureza, email, beneficios, area_id, url, endereco, bairro, municipio, cep, telefone, fim_de_semana, convenio, expira, seguro, observacoes);
        if (!success) {
            return res.status(404).json({ error: 'Instituicao not found' });
        }
        res.status(204).end();
