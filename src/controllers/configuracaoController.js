...
export const updateConfiguracao = async (req, res) => {
    try {
        const { id } = req.params;
        const { instituicao, mural_periodo_atual, curso_turma_atual, curso_abertura_inscricoes, curso_encerramento_inscricoes, termo_compromisso_periodo, termo_compromisso_inicio, termo_compromisso_final, periodo_calendario_academico } = req.body;
        // console.log("Updating configuracao with ID:", id);
        await Configuracao.update(instituicao, mural_periodo_atual, curso_turma_atual, curso_abertura_inscricoes, curso_encerramento_inscricoes, termo_compromisso_periodo, termo_compromisso_inicio, termo_compromisso_final, periodo_calendario_academico, id);
        res.status(204).end();
    } catch (error) {
