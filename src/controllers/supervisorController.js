...
export const createSupervisor = async (req, res) => {
    try {
        const { nome, email, celular, cress, regiao, cpf, escola, ano_formacao, cargo, observacoes } = req.body;
        const supervisor = await Supervisor.create(nome, email, celular, cress, regiao, cpf, escola, ano_formacao, cargo, observacoes);
...
export const updateSupervisor = async (req, res) => {
    try {
        const { id } = req.params;
        const { nome, email, celular, cress, regiao, cpf, escola, ano_formacao, cargo, observacoes } = req.body;
        const success = await Supervisor.update(id, nome, email, celular, cress, regiao, cpf, escola, ano_formacao, cargo, observacoes);
        if (!success) {
            return res.status(404).json({ error: 'Supervisor not found' });
        }
        res.status(204).end();
