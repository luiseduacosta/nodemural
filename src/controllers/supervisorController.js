...
export const createSupervisor = async (req, res) => {
    try {
        const { nome, email, telefone, celular, cress, regiao, cpf, escola, ano_formacao, cargo, observacoes, user_id } = req.body;
        const parsedUserId = user_id !== undefined && user_id !== null && user_id !== '' ? parseInt(user_id, 10) : null;
        const supervisor = await Supervisor.create(nome, cress, regiao, cpf, email, telefone, celular, escola, ano_formacao, cargo, observacoes, parsedUserId);
...
export const updateSupervisor = async (req, res) => {
    try {
        ...
        const success = await Supervisor.update(id, nome, cress, regiao, cpf, email, telefone, celular, escola, ano_formacao, cargo, observacoes, parsedUserId);
        if (!success) {
            return res.status(404).json({ error: 'Supervisor not found' });
        }
        res.status(204).end();
