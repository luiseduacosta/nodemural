...
app.get("/alunos/:id", async (req, res) => {
  ...
  try {
    ...
    const rows = await conn.query(
      "SELECT id, nome, nomesocial, registro, email, ingresso, telefone, celular, cpf, identidade, orgao, nascimento, cep, endereco, municipio, bairro, observacoes FROM alunos WHERE id = ?",
      [alunoId],
    );
    if (rows.length === 0) {
...
app.get("/alunos", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    let query =
      "SELECT id, nome, nomesocial, registro, email, ingresso, telefone, celular, cpf, identidade, orgao, nascimento, cep, endereco, municipio, bairro, observacoes FROM alunos ORDER BY nome ASC";
    let params = [];

    if (req.query.search) {
      query +=
        " WHERE nome LIKE ? OR nomesocial LIKE ? OR registro LIKE ? OR email LIKE ?";
...
app.get("/docentes", async (req, res) => {
  ...
  try {
    ...
    if (req.query.search) {
      query +=
        " WHERE nome LIKE ? OR siape LIKE ? OR email LIKE ? OR celular LIKE ?";
      const searchTerm = `%${req.query.search}%`;
      params = [searchTerm, searchTerm, searchTerm, searchTerm];
    }
...
app.get("/docentes/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query(
      "SELECT id, nome, siape, email, celular FROM docentes WHERE id = ?",
      [req.params.id],
    );
    if (rows.length === 0) {
...
app.get("/estagio/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query("SELECT * FROM estagio WHERE id = ?", [
      req.params.id,
    ]);
    if (rows.length === 0) {
...
app.get("/estagio/:id/supervisores", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query(
      "SELECT * FROM supervisores WHERE id IN (SELECT supervisor_id FROM inst_super WHERE instituicao_id = ?) ORDER BY nome ASC",
      [req.params.id],
    );
    if (rows.length === 0) {
...
app.delete("/supervisores/:id", async (req, res) => {
  ...
  try {
    ...
    await conn.query("DELETE FROM inst_super WHERE supervisor_id = ?", [
      req.params.id,
    ]);
...
app.delete(
  "/supervisores/:id/instituicoes/:instituicaoId",
  async (req, res) => {
    let conn;
    try {
      conn = await pool.getConnection();
...
app.get("/mural/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query("SELECT * FROM mural_estagio WHERE id = ?", [
      req.params.id,
    ]);
    if (rows.length === 0) {
  ...
  try {
    ...
    const result = await conn.query(
      ...
      [
        ...
        req.body.vagas,
        req.body.beneficios,
        req.body.final_de_semana,
        req.body.cargaHoraria,
        req.body.requisitos,
        req.body.turmaestagio_id,
        req.body.horario,
        req.body.professor_id,
        req.body.dataSelecao,
        req.body.dataInscricao,
        req.body.horarioSelecao,
        req.body.localSelecao,
        req.body.formaSelecao,
        req.body.contato,
        req.body.outras,
        req.body.periodo,
        req.body.datafax,
        req.body.localInscricao,
        req.body.email,
      ],
    );
    res.status(201).json({ id: Number(result.insertId) });
...
app.post("/inscricoes", async (req, res) => {
  ...
  try {
    ...
    if (existing.length > 0) {
      return res
        .status(400)
        .json({ error: "Aluno já inscrito nesta vaga para este período" });
    }
...
app.get("/visitas/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query(
      "SELECT visita.*, estagio.instituicao FROM visita left join estagio on visita.instituicao_id = estagio.id WHERE visita.id = ?",
      [req.params.id],
    );
    if (rows.length === 0) {
...
app.get("/estagiarios/:id/next-nivel", async (req, res) => {
  ...
  try {
    ...
    const estagiarioRows = await conn.query(
...
app.get("/turma_estagios", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query(
      "SELECT * FROM turma_estagios ORDER BY area ASC",
    );
    return res.json(rows);
  } catch (err) {
    return res.status(500).json({ error: err.message });
...
app.get("/turma_estagios/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query("SELECT * FROM turma_estagios WHERE id = ?", [
      req.params.id,
    ]);
    if (rows.length === 0) {
...
app.put("/turma_estagios/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query("UPDATE turma_estagios SET area = ? WHERE id = ?", [
      req.body.area,
      req.params.id,
    ]);
    res.status(204).end();
...
app.delete("/turma_estagios/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query("DELETE FROM turma_estagios WHERE id = ?", [
      req.params.id,
    ]);
    res.status(204).end();
  } catch (err) {
...
app.put("/configuracoes", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No configuration record to update" });
    }
    const id = rows[0].id;
...
app.delete("/atividades/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query("DELETE FROM folhadeatividades WHERE id = ?", [
      req.params.id,
    ]);
    res.status(204).end();
  } catch (err) {
...
// --- QUESTIONARIOS ENDPOINTS ---

// READ ALL QUESTIONARIOS (Simple list)
app.get("/questionarios", async (req, res) => {
  let conn;
  try {
    conn = await pool.getConnection();
    const rows = await conn.query(
      "SELECT id, title FROM questionarios ORDER BY title ASC",
    );
    return res.json(rows);
  } catch (err) {
    return res.status(500).json({ error: err.message });
...
app.get("/questao", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    let query = `
      SELECT q.*, qu.title as questionario_title 
      FROM questoes q
      LEFT JOIN questionarios qu ON q.questionario_id = qu.id
    `;
    let params = [];
    ...
    query += " ORDER BY q.questionario_id ASC, q.ordem ASC, q.id DESC";
...
app.get("/questao/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const rows = await conn.query("SELECT * FROM questoes WHERE id = ?", [
      req.params.id,
    ]);
    if (rows.length === 0) {
...
app.post("/questao", async (req, res) => {
  ...
  try {
    ...
    const result = await conn.query(
      ...
      [
        req.body.questionario_id,
        req.body.text,
        req.body.type,
        req.body.options, // Assuming this is passed as a generic string (JSON)
        req.body.ordem || 0,
        req.body.created,
        req.body.modified,
      ],
...
app.put("/questao/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query(
      "UPDATE questoes SET questionario_id = ?, text = ?, type = ?, options = ?, ordem = ?, modified = ? WHERE id = ?",
      [
        newData.questionario_id,
        newData.text,
        newData.type,
        newData.options,
        newData.ordem,
...
app.delete("/questao/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query("DELETE FROM questoes WHERE id = ?", [req.params.id]);
    res.status(204).end();
  } catch (err) {
    res.status(500).json({ error: err.message });
  } finally {
